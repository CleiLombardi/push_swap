...
/*   Updated: 2026/04/12 16:15:02 by roblomba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_finish_rotate_b(t_list **stack_b, t_list *ch_node, int size_b)
{
	...
	if (ch_node->target_pos <= size_b / 2)
	{
	...
	{
		remain = size_b - ch_node->target_pos;
		while (remain > 0)
		{
			rrb(stack_b);
			remain --;
		}
		ch_node->target_pos = 0;
	}
}
...
void	ft_do_cheapest_move(t_list **stack_a, t_list **stack_b)
{
	...
	orig_pos = ch_node->pos;
	orig_target = ch_node->target_pos;
	ft_rotate_both(stack_a, stack_b, ch_node);
	if (orig_pos > size_a / 2 && orig_target > size_b / 2)
	{
		while (ch_node->pos-- > 0)
			rra(stack_a);
		while (ch_node->target_pos-- > 0)
